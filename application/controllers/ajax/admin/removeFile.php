<?php
	class Controller_ajax_admin_removeFile extends Controller_ajax
	{
		public function get_index($id = null)
		{
			...
			{
				$user = $file->getUser();
				if(!empty($this->_current_member) && ($this->_current_member->getId() == $user->getId() || $this->_current_member->getPermission('can_remove')))
				{
					$path = $file->getPath();
					if(is_file($path))
					{
			...
			{
				$arrayInfo = [
					'infos_message' => 'Une erreur est survenue. Vous cherchez à supprimer une image inexistante.',
					'infos_message_status' => 'class="message infos error"',
				];
				$infos_message = \Eliya\Tpl::get('infos_message', $arrayInfo);
