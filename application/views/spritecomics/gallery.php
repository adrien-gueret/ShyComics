...
<?php if(!empty($view->user_files)): ?>
	<?php foreach($view->user_files as $key => $file) : ?>
		...
		<br /><a href="<?= $view->base_url . 'spritecomics/gallery/file/' . $file->getId() ?>"><img src="<?= $view->base_url; ?>public/images/file.png" class="galleryFile" alt="Dossier" /></a>
		<?= $file->prop('name'); ?>
		<?php else: ?>
		<br /><a href="<?= $view->base_url . 'spritecomics/gallery/file/' . $file->getId() ?>"><img src="<?= $view->base_url . $file->getPath(); ?>" alt="<?= $file->prop('name'); ?>" title="<?= $file->prop('name'); ?>" /></a>
		<?php endif; ?>
	<?php endforeach; ?>
<?php else: ?>
